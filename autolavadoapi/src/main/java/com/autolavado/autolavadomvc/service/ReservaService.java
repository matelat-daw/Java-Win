...
public class ReservaService {
    ...
    public ReservaServicio crearReserva(ReservaForm form) {
    ...
    public Page<ReservaServicio> buscarPagina(String matricula, boolean soloPendientes, String sortField, String dir, int page, int size) {
        int safePage = Math.max(0, page);
        int safeSize = size <= 0 ? 8 : Math.min(size, 100);

        if ("precio".equalsIgnoreCase(sortField)) {
            List<ReservaServicio> sorted = buscarPorFiltros(matricula, soloPendientes, sortField, dir);
            int fromIndex = safePage * safeSize;
            if (fromIndex >= sorted.size()) {
                return new PageImpl<>(List.of(), PageRequest.of(safePage, safeSize), sorted.size());
            }
        ...
        Pageable pageable = PageRequest.of(safePage, safeSize, mapSort(sortField, dir));

        boolean hasMatricula = matricula != null && !matricula.isBlank();
        if (soloPendientes) {
            if (hasMatricula) {
                return repository.findByMatriculaContainingIgnoreCaseAndEstado_EstadoIgnoreCase(matricula, "PENDIENTE", pageable);
            }
            return repository.findByEstado_EstadoIgnoreCase("PENDIENTE", pageable);
        }

        if (hasMatricula) {
            return repository.findByMatriculaContainingIgnoreCase(matricula, pageable);
        }
        return repository.findAll(pageable);
    }

    private Sort mapSort(String sortField, String dir) {
