...
App.getInstance().navigateTo('/dashboard');
            ...
            await Utils.loadHtml(this.selector, '/frontend/components/login/login.component.html');
...
Utils.showMessage('Error', 'No se pudo cargar el formulario de login', 'error');
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            const backendMessage = String(error?.backendMessage || error?.backendPayload?.message || error?.message || '');
            ...
            } else if (normalizedMessage.includes('no ha sido verificada') || normalizedMessage.includes('verificada')) {
                errorTitle = 'Cuenta no verificada';
                errorMessage = `
                    <div class="text-start">
                        <p class="mb-2"><strong>Estas seguro que validaste tu registro?</strong></p>
                        <p class="mb-2">Tu cuenta aun no ha sido confirmada desde el enlace que enviamos por email.</p>
                        <p class="mb-0">Revisa tu bandeja de entrada y tambien la carpeta de spam. Cuando completes la verificacion podras iniciar sesion normalmente.</p>
                    </div>
                `;
