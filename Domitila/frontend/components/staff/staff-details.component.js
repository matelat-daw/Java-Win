...
App.getInstance().navigateTo('/login');
            ...
            if (staffRole !== 'ADMIN' && staffRole !== 'PREMIUM') {
...
this.show('ERROR: ' + e.message);
    ...
    async loadStaff() {
const res = await StaffService.getStaffById(this.staffId);
        this.staff = res && res.data ? res.data : res.staff;
        if (!this.staff) throw new Error('Staff no encontrado');
}

    render() {
        const admin = AuthService.getStaffSession();
        const adminName = admin ? `${admin.name} ${admin.surname1}` : 'Admin';
        const currentStaffRole = AuthService.getRole();
        const isAdmin = currentStaffRole === 'ADMIN';
        
        // Determinar imagen del usuario
        const staffImg = this.staff.profileImg ? 
            `${API_CONFIG.BASE_URL}/images/${this.staff.profileImg}` :
            null;
        ...
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h1 class="display-5 fw-bold mb-0">
                    <i class="fas fa-id-card me-3 text-primary"></i>Detalles de Staff
                </h1>
                <p class="text-muted mt-2">Gestionando el perfil de <strong>${this.staff.nick}</strong></p>
            </div>
        ...
        <div class="col-lg-4 mb-4">
            <div class="card shadow-sm text-center">
                <div class="card-body py-5">
                    <div class="mb-4">
                        ${staffImg ? 
                            `<img src="${staffImg}" alt="${this.staff.nick}" class="img-thumbnail rounded-circle shadow" style="width: 150px; height: 150px; object-fit: cover; border: 5px solid #fff;">` :
                    ...
                    <h3 class="fw-bold mb-1">${this.staff.name} ${this.staff.surname1}</h3>
                    <p class="text-muted mb-3">@${this.staff.nick}</p>
                    <span class="badge ${this.getRoleBadgeClass(this.staff.role)} fs-6 px-3 py-2">
                ...
                <div class="card-body">
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <p class="fs-5"><i class="fas fa-envelope me-2 text-muted"></i>${this.staff.email}</p>
                        </div>
                        <div class="col-md-6">
                            <label class="text-muted small text-uppercase fw-bold">Teléfono</label>
                            <p class="fs-5"><i class="fas fa-phone me-2 text-muted"></i>${this.staff.phone || 'No proporcionado'}</p>
                        </div>
                        <div class="col-md-6">
                            <label class="text-muted small text-uppercase fw-bold">Género</label>
                            <p class="fs-5">
                                <i class="fas fa-venus-mars me-2 text-muted"></i>${this.staff.gender || 'No especificado'}
                            </p>
                        </div>
                        <div class="col-md-6">
                            <label class="text-muted small text-uppercase fw-bold">Fecha de Nacimiento</label>
                            <p class="fs-5">
                                <i class="fas fa-calendar-alt me-2 text-muted"></i>${this.staff.bday ? new Date(this.staff.bday).toLocaleDateString() : 'N/A'}
                            </p>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <p class="fs-5">
                                <span class="badge ${this.staff.active ? 'bg-success' : 'bg-secondary'} me-2">
                                    ${this.staff.active ? 'ACTIVO' : 'INACTIVO'}
                                </span>
                                <span class="badge ${this.staff.emailVerified ? 'bg-primary' : 'bg-warning'}">
                                    ${this.staff.emailVerified ? 'EMAIL VERIFICADO' : 'PENDIENTE VERIFICAR'}
                                </span>
                            </p>
                        </div>
                        <div class="col-md-6">
                            <label class="text-muted small text-uppercase fw-bold">Fecha de Registro</label>
                            <p class="fs-5"><i class="fas fa-clock me-2 text-muted"></i>${new Date(this.staff.createdAt).toLocaleString()}</p>
                        </div>
                    </div>
                </div>
                <div class="card-footer bg-light py-3">
                    <small class="text-muted">
                        <i class="fas fa-history me-2"></i>Última actualización: ${new Date(this.staff.updatedAt).toLocaleString()}
                    </small>
                </div>
            </div>
        </div>
...
`;
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-danger">
                        ...
                        <div class="modal-body">
                            ...
                            <p>¿Estás seguro de que deseas eliminar al usuario <strong>${this.staff.nick}</strong>?</p>
    ...
    async delete() {
        try {
            ...
            const res = await StaffService.deleteStaff(this.staff.id);
            if (res && res.success) {
                // Cerrar modal
...
window.StaffDetailsComponent = StaffDetailsComponent;
if (typeof AppScripts !== 'undefined') {
AppScripts.register('staff-details');
} else {
}
