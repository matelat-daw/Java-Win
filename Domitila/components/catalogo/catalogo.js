...
export async function catalogoHtml({ products, imgBaseUrl, page = 1, pageSize = 8, isAdmin = false }) {
    ...
    const productsMobile = pageProducts.map((p) => {
        ...
        const acciones = buildAccionesHtml({ id: p.id, nombreJs: p.nombre, isAdmin });
    ...
    return renderTemplate(tpl, { productsMobile, productsTable, pagination, adminAddButton });
}
...
function buildAccionesHtml({ id, nombreJs, isAdmin }) {
    const safeId = escapeHtml(id);
    const safeNombreJs = escapeJsString(nombreJs);
    const view = `<button class="btn btn-sm btn-primary" onclick="navigate('detalle', {id: '${safeId}'})">👁️ Ver</button>`;
    if (!isAdmin) return view;
    const edit = `<button class="btn btn-sm btn-warning" onclick="navigate('editar', {id: '${safeId}'})">✏️ Editar</button>`;
    const del = `<button class="btn btn-sm btn-danger" onclick="confirmarEliminacion('${safeId}', '${safeNombreJs}')">🗑️ Eliminar</button>`;
