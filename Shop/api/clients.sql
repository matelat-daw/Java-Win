...
-- Tiempo de generación: 01-06-2026 a las 22:08:44
...
--
CREATE DATABASE IF NOT EXISTS `clients` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_uca1400_nopad_ai_ci;
USE `clients`;
...
INSERT INTO `user` (`id`, `nick`, `name`, `surname1`, `surname2`, `bday`, `phone`, `gender`, `email`, `password`, `profileImg`, `active`, `emailVerified`, `verificationToken`, `verificationTokenExpiry`, `createdAt`, `updatedAt`) VALUES
...
INSERT INTO `user_role` (`id`, `userId`, `roleId`, `assignedAt`) VALUES
...
ALTER TABLE `user_role`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_user_role` (`userId`,`roleIdd`),
  ADD KEY `idx_user_id` (`userId`),
  ADD KEY `idx_role_id` (`roleId`);
...
--
ALTER TABLE `user_role`
  ADD CONSTRAINT `1` FOREIGN KEY (`userId`) REFERENCES `user` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `2` FOREIGN KEY (`roleId`) REFERENCES `role` (`id`) ON DELETE CASCADE;
COMMIT;
