...
public class SecurityConfig {
        ...
        return username -> userRepository.findByEmail(username)
            .map(user -> {
                List<GrantedAuthority> authorities = user.getRoles().stream()
                    .map(role -> new SimpleGrantedAuthority(toAuthorityName(role == null ? null : role.getName())))
    ...
    private static String toAuthorityName(String roleName) {
        if (roleName == null) return "ROLE_USER";
        String trimmed = roleName.trim();
        if (trimmed.isEmpty()) return "ROLE_USER";
        if (trimmed.regionMatches(true, 0, "ROLE_", 0, 5)) return "ROLE_" + trimmed.substring(5).toUpperCase();
    ...
    public SecurityFilterChain filterChain(
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/products/**", "/api/categorias", "/api/imgs/**").permitAll()
